...
class Agenda extends Component {
    constructor(props) {
        ...
        this.onVisibleMonthsChange = (months) => {
            ...
            if (this.props.items && !this.state.firstReservationLoad) {
                ...
                this.scrollTimeout = setTimeout(() => {
                    var _a, _b;
                    if (this._isMounted) {
                        (_b = (_a = this.props).loadItemsForMonth) === null || _b === void 0 ? void 0 : _b.call(_a, months[0]);
                    }
                }, 200);
            }
        };
        ...
        this.renderWeekDaysNames = () => {
            return (React.createElement(WeekDaysNames, { firstDay: this.props.firstDay, style: this.style.dayHeader }));
        };
        this.renderWeekNumbersSpace = () => {
            return this.props.showWeekNumbers && React.createElement(View, { style: this.style.dayHeader });
        };
    ...
    renderReservations() {
        ...
        return (React.createElement(ReservationList, Object.assign({}, reservationListProps, { ref: this.list, selectedDay: this.state.selectedDay, topDay: this.state.topDay, onDayChange: this.onDayChange })));
    }
    renderCalendarList() {
    ...
    renderKnob() {
        ...
        let knob = React.createElement(View, { style: this.style.knobContainer });
        if (!hideKnob) {
            const knobView = renderKnob ? renderKnob() : React.createElement(View, { style: this.style.knob });
            knob = !this.state.calendarScrollable || showClosingKnob ? (React.createElement(View, { style: this.style.knobContainer },
                React.createElement(View, { ref: this.knob }, knobView))) : null;
        }
        return knob;
    }
    render() {
        ...
        return (React.createElement(View, { testID: testID, onLayout: this.onLayout, style: [style, this.style.container] },
            React.createElement(View, { style: this.style.reservations }, this.renderReservations()),
            React.createElement(Animated.View, { style: headerStyle },
                React.createElement(Animated.View, { style: [this.style.animatedContainer, { transform: [{ translateY: contentTranslate }] }] }, this.renderCalendarList()),
                this.renderKnob()),
            React.createElement(Animated.View, { style: weekdaysStyle },
                this.renderWeekNumbersSpace(),
                this.renderWeekDaysNames()),
            React.createElement(Animated.ScrollView, { ref: this.scrollPad, style: [this.style.scrollPadStyle, scrollPadStyle], overScrollMode: "never", showsHorizontalScrollIndicator: false, showsVerticalScrollIndicator: false, scrollEventThrottle: 8, scrollsToTop: false, onTouchStart: this.onTouchStart, onTouchEnd: this.onTouchEnd, onScrollBeginDrag: this.onStartDrag, onScrollEndDrag: this.onSnapAfterDrag, onScroll: Animated.event([{ nativeEvent: { contentOffset: { y: this.state.scrollY } } }], { useNativeDriver: true }) },
                React.createElement(View, { testID: AGENDA_CALENDAR_KNOB, style: { height: agendaHeight + KNOB_HEIGHT }, onLayout: this.onScrollPadLayout }))));
    }
}

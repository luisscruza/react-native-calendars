...
const CalendarHeader = forwardRef((props, ref) => {
    ...
    const renderWeekDays = useMemo(() => {
        ...
        return dayNames.map((day, index) => {
            ...
            return (React.createElement(Text, { allowFontScaling: false, key: index, style: dayStyle, numberOfLines: 1, accessibilityLabel: '', testID: `${testID}.dayName_${day}` }, day));
        });
    ...
    const _renderHeader = () => {
        ...
        return (React.createElement(Fragment, null,
            React.createElement(Text, Object.assign({ allowFontScaling: false, style: style.current.monthText, testID: `${testID}.title` }, webProps), formatNumbers(month === null || month === void 0 ? void 0 : month.toString(monthFormat)))));
    };
    const _renderArrow = (direction) => {
        ...
        return (React.createElement(TouchableOpacity, { onPress: onPress, disabled: shouldDisable, style: style.current.arrow, hitSlop: hitSlop, testID: `${testID}.${arrowId}`, importantForAccessibility: 'no-hide-descendants' }, renderArrow ? renderArrow(arrowDirection) : React.createElement(Image, { source: imageSource, style: shouldDisable ? style.current.disabledArrowImage : style.current.arrowImage })));
    };
    const renderIndicator = () => {
        if (displayLoadingIndicator) {
            return (React.createElement(ActivityIndicator, { color: theme === null || theme === void 0 ? void 0 : theme.indicatorColor, testID: `${testID}.loader` }));
        }
    };
    const renderWeekNumbersSpace = () => {
        return showWeekNumbers && React.createElement(View, { style: style.current.dayHeader });
    };
    const renderDayNames = () => {
        if (!hideDayNames) {
            return (React.createElement(View, { style: dayNamesStyle, testID: `${testID}.dayNames`, importantForAccessibility: 'no-hide-descendants' },
                renderWeekNumbersSpace(),
                renderWeekDays));
        }
    };
    return (React.createElement(View, { testID: testID, style: propsStyle, accessible: true, accessibilityRole: 'adjustable', accessibilityActions: accessibilityActions, onAccessibilityAction: onAccessibilityAction, accessibilityElementsHidden: accessibilityElementsHidden, importantForAccessibility: importantForAccessibility, onLayout: onHeaderLayout },
        React.createElement(View, { style: headerStyle },
            _renderArrow('left'),
            React.createElement(View, { style: style.current.headerContainer, importantForAccessibility: 'no-hide-descendants' },
                _renderHeader(),
                renderIndicator()),
            _renderArrow('right')),
        renderDayNames()));

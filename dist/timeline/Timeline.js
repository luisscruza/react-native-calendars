...
const Timeline = (props) => {
    ...
    const renderEvents = (dayIndex) => {
        const events = packedEvents[dayIndex].map((event, eventIndex) => {
            ...
            return (React.createElement(EventBlock, { key: eventIndex, index: eventIndex, event: event, styles: styles.current, format24h: format24h, onPress: onEventPress, renderEvent: renderEvent, testID: `${testID}.event.${event.id}` }));
        });
        return (React.createElement(View, { pointerEvents: 'box-none', style: [{ marginLeft: dayIndex === 0 ? timelineLeftInset : undefined }, styles.current.eventsContainer] }, events));
    };
    const renderTimelineDay = (dayIndex) => {
        ...
        return (React.createElement(React.Fragment, { key: dayIndex },
            renderEvents(dayIndex),
            indexOfToday !== -1 && showNowIndicator && React.createElement(NowIndicator, { width: width / numberOfDays, left: left, styles: styles.current })));
    };
    ...
    , Object.assign({ 
        ...
        React.createElement(TimelineHours, { start: start, end: end, date: pageDates[0], format24h: format24h, styles: styles.current, unavailableHours: unavailableHours, unavailableHoursColor: unavailableHoursColor, onBackgroundLongPress: onBackgroundLongPress, onBackgroundLongPressOut: onBackgroundLongPressOut, width: width, numberOfDays: numberOfDays, timelineLeftInset: timelineLeftInset, testID: `${testID}.hours` }),

...
const TimelineHours = (props) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement(TouchableWithoutFeedback, { onLongPress: handleBackgroundPress, onPressOut: handlePressOut },
            React.createElement(View, { style: StyleSheet.absoluteFillObject })),
        unavailableHoursBlocks.map((block, index) => (React.createElement(View, { key: index, style: [
        ...
        hours.map(({ timeText, time }, index) => {
            return (React.createElement(React.Fragment, { key: time },
                React.createElement(Text, { key: `timeLabel${time}`, style: [styles.timeLabel, { top: offset * index - 6, width: timelineLeftInset - 16 }] }, timeText),
                time === start ? null : (React.createElement(View, { key: `line${time}`, testID: `${testID}.${time}.line`, style: [styles.line, { top: offset * index, width: dimensionWidth - EVENT_DIFF, left: timelineLeftInset - 16 }] })),
                React.createElement(View, { key: `lineHalf${time}`, testID: `${testID}.${time}.lineHalf`, style: [styles.line, { top: offset * (index + 0.5), width: dimensionWidth - EVENT_DIFF, left: timelineLeftInset - 16 }] })));
        }),
        times(numberOfDays, (index) => React.createElement(View, { key: index, style: [styles.verticalLine, { right: (index + 1) * width / numberOfDays }] }))));
};

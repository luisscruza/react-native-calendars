...
const ExpandableCalendar = forwardRef((props, ref) => {
    ...
    const scrollPage = useCallback((next, updateSource = UpdateSources.PAGE_SCROLL) => {
        if (horizontal) {
            ...
            setDate === null || setDate === void 0 ? void 0 : setDate(toMarkingFormat(d), updateSource);
        }
    ...
    const _renderArrow = useCallback((direction) => {
        ...
        return (React.createElement(Image, { source: direction === 'right' ? rightArrowImageSource : leftArrowImageSource, style: style.current.arrowImage, testID: `${testID}.${direction}Arrow` }));
    }, [renderArrow, rightArrowImageSource, leftArrowImageSource, testID]);
    const renderWeekDaysNames = () => {
        return (React.createElement(View, { style: weekDaysStyle },
            React.createElement(WeekDaysNames, { firstDay: firstDay, style: style.current.dayHeader })));
    };
    const renderAnimatedHeader = () => {
        var _a;
        const monthYear = (_a = new XDate(date)) === null || _a === void 0 ? void 0 : _a.toString('MMMM yyyy');
        return (React.createElement(Animated.View, { ref: header, style: animatedHeaderStyle, pointerEvents: 'none' },
            React.createElement(Text, { allowFontScaling: false, style: style.current.headerTitle }, monthYear),
            renderWeekDaysNames()));
    };
    const renderKnob = () => {
        return (React.createElement(View, { style: style.current.knobContainer, pointerEvents: 'box-none' },
            React.createElement(TouchableOpacity, { style: style.current.knob, testID: `${testID}.knob`, onPress: toggleCalendarPosition, hitSlop: knobHitSlop })));
    };
    const renderWeekCalendar = () => {
        const WeekComponent = disableWeekScroll ? Week : WeekCalendar;
        return (React.createElement(Animated.View, { ref: weekCalendarWrapper, style: weekCalendarStyle, pointerEvents: isOpen ? 'none' : 'auto' },
            React.createElement(WeekComponent, Object.assign({ testID: `${testID}.weekCalendar`, firstDay: firstDay }, others, { allowShadow: disableWeekScroll ? undefined : false, current: disableWeekScroll ? date : undefined, theme: themeObject, style: calendarStyle, hideDayNames: true, onDayPress: _onDayPress, accessibilityElementsHidden // iOS

...
const WeekCalendar = (props) => {
    ...
    const renderItem = useCallback(({ item }) => {
        ...
        return (React.createElement(Week, Object.assign({}, others, { markedDates: markings, current: item, firstDay: firstDay, style: weekStyle, context: currentContext, onDayPress: _onDayPress, numberOfDays: numberOfDays, timelineLeftInset: timelineLeftInset })));
    }, [firstDay, _onDayPress, context, date, markedDates]);
    const keyExtractor = useCallback((item, index) => `${item}-${index}`, []);
    const renderWeekDaysNames = useMemo(() => {
        return (React.createElement(WeekDaysNames, { firstDay: firstDay, style: style.current.dayHeader }));
    }, [firstDay]);
    ...
    return (React.createElement(View, { testID: testID, style: weekCalendarStyle },
        !hideDayNames && (React.createElement(View, { style: containerStyle }, renderWeekDaysNames)),
        React.createElement(View, { style: style.current.container },
            React.createElement(FlatList, { testID: `${testID}.list`, ref: list, style: style.current.container, data: listData, horizontal: true, showsHorizontalScrollIndicator: false, pagingEnabled: true, scrollEnabled: true, renderItem: renderItem, keyExtractor: keyExtractor, initialScrollIndex: NUMBER_OF_PAGES, getItemLayout: getItemLayout, viewabilityConfigCallbackPairs: viewabilityConfigCallbackPairs.current, onEndReached: onEndReached, onEndReachedThreshold: 1 / NUM_OF_ITEMS }))));
};

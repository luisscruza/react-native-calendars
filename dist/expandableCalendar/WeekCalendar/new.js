...
const WeekCalendar = (props) => {
    ...
    const renderItem = useCallback((_type, item) => {
        ...
        return (React.createElement(Week, Object.assign({}, others, { key: item, current: isSameWeek ? date : item, firstDay: firstDay, style: weekStyle, markedDates: markedDates, onDayPress: onDayPress, context: context })));
    }, [date, markedDates]);
    return (React.createElement(View, { testID: testID, style: [allowShadow && style.current.containerShadow, !hideDayNames && style.current.containerWrapper] },
        !hideDayNames && (React.createElement(View, { style: [style.current.week, style.current.weekCalendar] },
            React.createElement(WeekDaysNames, { firstDay: firstDay, style: style.current.dayHeader }))),
        React.createElement(View, null,
            React.createElement(InfiniteList, { key: "week-list", isHorizontal: true, ref: list, data: items, renderItem: renderItem, reloadPages: reloadPages, onReachNearEdgeThreshold: Math.round(NUMBER_OF_PAGES * 0.4), extendedState: extraData, style: style.current.container, initialPageIndex: NUMBER_OF_PAGES, pageHeight: calendarHeight, pageWidth: containerWidth, onPageChange: onPageChange, scrollViewProps: {
